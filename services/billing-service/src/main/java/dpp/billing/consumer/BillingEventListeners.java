...
public class BillingEventListeners {
    ...
    public void onEndorsement(String message) {
        try {
            JsonNode n = objectMapper.readTree(message);
            adjustmentService.applyEndorsement(UUID.fromString(n.get("policy_id").asText()),
    ...
    public void onCancellation(String message) {
        try {
            JsonNode n = objectMapper.readTree(message);
            adjustmentService.applyCancellation(UUID.fromString(n.get("policy_id").asText()),
